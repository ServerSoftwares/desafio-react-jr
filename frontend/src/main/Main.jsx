...
function attProd(){
    ...
    var novoProd = document.getElementById("NovoProd").value

    /*Axios.patch("http://localhost:3001/produtos",obj).then(resp => {
        console.log(resp)
        att()
    })*/
}
...
<div id="containerProdutos">{listaProdutos.map((value,key)=>{
            ...
            return <Produto key = {value.codigo} codigo={value.codigo} descricao={value.descricao} preco={value.preco} data_cadastro={value.data_cadastro} excluir={()=>excluirProd(value.codigo)} attProd={()=>attProd()}></Produto>})}</div>
